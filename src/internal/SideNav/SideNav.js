    ...
    return Object.keys(subnav).map(subNavItem => {
      ...
      return (
        <li role="menuitem" key={subNavItem} className={classNames} tabIndex="-1">
          <Link className="sub-nav__item-link" aria-label={subnav[subNavItem]} to={link} tabIndex={tabIndex}>{subnav[subNavItem]}</Link>
        </li>
      );
    });
  }
    ...
    return (
      <SideNavItem key={parentItem} isCurrentPath={isCurrentPath}>
        <p className="main-nav-item__heading">{SiteNavStructure[parentItem].title}
          <Icon className="main-nav-item__arrow" name="caret--down" aria-hidden="true" description="Menu arrow icon" />
        </p>
        <ul role="menu" aria-label={`${SiteNavStructure[parentItem].title} sub menu`} aria-hidden="true" className="main-nav__sub-nav">
          {subNavItems}
        </ul>
      </SideNavItem>
    );
  ...
  render() {
    ...
    return (
      <nav role="navigation" aria-label="Page Navigation" aria-expanded={isOpen} className={classNames}>
        <a id="skip-to-content" tabIndex="0" role="button" className="skip-to-content" onKeyDown={this.handleSkip}>Skip to main content</a>
        ...
        <footer className="side-nav__footer" role="contentinfo">
