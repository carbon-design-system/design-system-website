  ...
  render() {
    ...
    if (componentInfo.variations) {
      componentContent = Object.keys(componentInfo.variations).map(variation =>
        this.renderVariation(component, variation, componentInfo.variations[variation], codepenSlug)
      );
    } else {
      let htmlFile;
    ...
    return (
      <div className="page code-page test">
        ...
        {componentContent}
        {hasReactVersion && (
          <div className="code-page__react-version">
            <svg
              width="32"
              height="32"
              viewBox="0 0 256 230"
            ...
            <div>
              <p>
                Looking for the React component?{' '}
                <a target="_blank" href={`http://react.carbondesignsystem.com/?selectedKind=${componentNameLink}`}>
                  View {componentName} in our React Storybook.
                </a>
              </p>
            </div>
          </div>
