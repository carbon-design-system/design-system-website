...
class GlobalSearch extends Component {
  ...
  render() {
    ...
    const topResultsClasses = classNames({
      ...
      "global-search__results--section--hidden": this.props.results.length === 0
    });
    ...
    const resultsHTML =
      ...
      this.props.val.length > 2 ? (
        ...
        <div className="global-search__results">
            ...
            {Object.keys([...this.props.results]).map(item => {
              ...
              const componentName =
                result.child
                  .replace(/-/g, " ")
                  .charAt(0)
                  .toUpperCase() + result.child.replace(/-/g, " ").substring(1);
              const resultLink =
          ...
          >
            <h4 className="global-search__sub-heading">{relatedResultsText}</h4>
            {Object.keys([...this.props.relatedResults]).map((item, key) => {
              ...
              if (toHTML.indexOf(this.props.currentQuery) < 0) {
                if (toHTML.indexOf(this.props.currentQuery.toLowerCase()) < 0) {
                  indexNumber = -1;
                } else {
              ...
              return (
                <Link key={item} to={resultLink} className={resultClasses}>
                  <p className="related-results__link">
                    <span className="bx--link">{resultLinkText}</span>
                  </p>
                  <p className="result-content">
                    <span>{firstSubstring}</span>
                    <span className="query">{highlightedWord}</span>
                    <span>{lastSubstring}</span>
                  </p>
                </Link>
              );
            })}
    ...
    return <div className={searchClasses}>{resultsHTML}</div>;
  }
}
